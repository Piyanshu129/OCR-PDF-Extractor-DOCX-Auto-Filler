...
#
#
# # Set Poppler path (macOS)
# poppler_path = "/opt/homebrew/bin"
# os.environ["PATH"] += os.pathsep + poppler_path
#
# # Ensure output folder exists
# os.makedirs("filled_docs", exist_ok=True)
#
# # ---------- Field Extraction from Ticket PDF ----------
...
# import os
#
# # def fill_docx_template(customer_name, data, booking_number):
# #     template_path = f"templates/{customer_name.upper().strip()}.docx"
# #     if not os.path.exists(template_path):
# #         return f"❌ Template for '{customer_name}' not found."
# #
# #     doc = Document(template_path)
# #     date_time_weighing = f"{data.get('DATE OUT', '')}      {data.get('TIME OUT', '')}"
# #     today = datetime.today().strftime("%d.%m.%Y")
# #
# #     # What to fill in the "Particulars" column
# #     replacements = {
...
# #         "CARGO WT": data.get("NET CARGO WEIGHT", ""),
# #         "TARE  WT": data.get("CONTAINER TARE WT TOTAL", ""),
# #         "VGM  WEIGHT": data.get("GROSS WEIGHT", ""),
...
# #             if len(row.cells) < 2:
# #                 continue
# #             label_cell = row.cells[1] if len(row.cells) > 2 else row.cells[0]  # Label column
# #             value_cell = row.cells[2] if len(row.cells) > 2 else row.cells[1]  # Value column
# #             label = label_cell.text.strip()
# #
# #             for key, val in replacements.items():
# #                 if key.lower() in label.lower():
# #                     value_cell.text = str(val)
# #
# #     output_path = f"filled_docs/filled_{customer_name.replace(' ', '_')}.docx"
...
#         ("TARE  WT", "TARE  WEIGHT"): data.get("CONTAINER TARE WT TOTAL", ""),
#         ("VGM  WT", "VGM  WEIGHT"): data.get("GROSS WEIGHT", ""),
#     }
#
#     for table in doc.tables:
#         for row in table.rows:
...
#                             new_lines = []
#                             for line in lines:
#                                 if any(label.lower() in line.lower() for label in keys_tuple):
#                                     matched_label = next(lbl for lbl in keys_tuple if lbl.lower() in line.lower())
#                                     new_lines.append(f"{matched_label} :    {weight_val}")
#                                 else:
#                                     new_lines.append(line)
#                             cell.text = "\n".join(new_lines)
#                             break  # Don't check other variants once matched
#
...
#     st.image(first_image, caption="Ticket Page 1", use_container_width=True)
#     ticket_text = pytesseract.image_to_string(first_image) + "\n"
#
#     # st.subheader("📜 OCR Text (Ticket)")
#     # st.text_area("Full OCR Text", ticket_text, height=250)
#
...
#     booking_text = ""
#     first_image = images[0]
#     st.image(first_image, caption="Booking Page 1", use_container_width=True)
#     booking_text = pytesseract.image_to_string(first_image) + "\n"
#
...
#         result = fill_docx_template(customer_name, ticket_data, booking_no)
